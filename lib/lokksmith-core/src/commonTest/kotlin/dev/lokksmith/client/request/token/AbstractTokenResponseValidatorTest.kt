...
abstract class AbstractTokenResponseValidatorTest<T : IdToken?> {
    ...
    private fun `test validate`(
        ...
        val idToken = Jwt(header = Jwt.Header(alg = "none"), payload = idTokenPayload)
    ...
    fun `validate should succeed`() = runTest {
        `test validate`(client = createTestClient { copy(nonce = "0D1ck61") })
    }
        ...
        val e =
            assertFailsWith<IllegalArgumentException> {
                `test validate`(
                    client =
                        createTestClient {
                            copy(metadata = metadata.copy(issuer = "another issuer"))
                        }
                )
            }
    ...
    fun `validate should fail on client ID mismatch`() = runTest {
        val e =
            assertFailsWith<IllegalArgumentException> {
                `test validate`(client = createTestClient { copy(id = "another client ID".asId()) })
            }
