 ...
 *
 * This use case wraps [Client.runWithTokens], attempting to execute the provided lambda with
 * fresh and valid [Tokens]. If the OpenID provider responds with an OAuth error matching one
 * of [errors] (by default, only `invalid_grant`), the client’s local session is reset via
 * [Client.resetTokens], effectively logging out the user on this device. Other exceptions or
 * errors are not handled and will be rethrown.
 *
 * **Note:** Server-side invalidation is only detected when a token refresh is required.
 * If the tokens are still valid and do not need to be refreshed, the body of [invoke] is executed
 * and no invalidation check occurs.
 *
 * Typical usage is to ensure that if the user's session is revoked or otherwise invalidated
 * on the provider, the local client state is kept in sync by removing tokens and requiring
 * re-authentication.
 *
...
public class RunWithTokensOrResetUseCase(
     ...
     * detected automatically. To force a reset from your own code, throw a [ResetClientStateException].
     *
